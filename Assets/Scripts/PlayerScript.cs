    ...
    void Update()
    {
        ...
        if (PV.IsMine)
        {
            ...
            if (ismove )
            {
                ...
                Debug.Log("right:" + maxSpeed_right);
                Debug.Log("left:" + maxSpeed_left);
                axis = Input.GetAxisRaw("Horizontal");
                RB.AddForce(Vector2.right * axis, ForceMode2D.Impulse);
                if (RB.velocity.x > maxSpeed_right)
                    RB.velocity = new Vector2(maxSpeed_right, RB.velocity.y);
                else if (RB.velocity.x < maxSpeed_left * (-1))
                    RB.velocity = new Vector2(maxSpeed_left * (-1), RB.velocity.y);
                ...
                // 총알 발사
                if (Input.GetMouseButtonDown(0) && (BulletImage.fillAmount >= Bullet) && isCanShoot && isShoot && Liflenum==0)
                {
                    ...
                    PhotonNetwork.Instantiate("Bullet", Gun.position, Quaternion.identity).GetComponent<PhotonView>().RPC("DirRPC", RpcTarget.All, 1, (int)GN.GetFloat("Blend"), (int)Gun.transform.rotation.eulerAngles.z, 10, actor);
                ...
                {
                    if (maxis < 0) { PV.RPC("ParticleRPC", RpcTarget.All, 0); RB.AddForce(Vector2.right * 200); }
                    else {PV.RPC("ParticleRPC", RpcTarget.All, 1); RB.AddForce(Vector2.left * 200); }
                    BulletImage.fillAmount -= 0.03f;
                    PhotonNetwork.Instantiate("Bullet", Gun.position, Quaternion.identity).GetComponent<PhotonView>().RPC("DirRPC", RpcTarget.All, 1, 0, (int)Gun.transform.rotation.eulerAngles.z+ Random.Range(-5, 6), 15, actor);
                    AN.SetTrigger("shot"); //총알 발사 
                ...
                if (Liflenum>0 && BulletImage.fillAmount <= 0)
                {
